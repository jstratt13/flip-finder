...
const NOUNS = new Set(vocab.nouns.filter((n) => !n.includes(' ')));
...
export function judgeResult(resultTitle, id) {
  const toks = tokens(resultTitle);
  const set = new Set(toks.map((x) => x.t));
  const norm = ` ${normalize(resultTitle).replace(/[-/]/g, ' ')} `;
  const squashed = normalize(resultTitle).replace(/[^a-z0-9]/g, '');
  ...
  const codeHits = id.codes.filter(
    (c) => set.has(c) || (c.length >= 5 ? squashed.includes(c) : !new RegExp(`[0-9a-z]${c}|${c}[0-9a-z]`).test(squashed) && squashed.includes(c))
  );
  const genOk = id.generations.every((g) => {
    const [w, n] = g.split(' ');
    return new RegExp(`(^|\\s)${w}\\s?${n}(\\s|$)`).test(norm);
  });
  ...
  if (id.codes.length || id.generations.length) {
    const slashLists = normalize(resultTitle).split(' ').filter((w) => w.includes('/'));
